    ...
    <body>
        <div class="container">
            <div id="display-frame" class="row">
                ...
                <h1 align="center" class="fancy-text">Hangman: Wild West</h1>
...
<script type="text/javascript">
    ...
    state.user_state = user_state;  // stores multi game user statistics
    ...
    function start_screen(word) { // load the start screen display
        ...
        display.innerHTML = '<div class="centered">' +
                                    ...
                                    generate_input_box() + '<br/>' +
                                    generate_check_button() +
                                    generate_user_display() +
                                     generate_hangman_pane() +
                              '</div>';
    }
    ...
    function select_img(incorrect_guesses) {
        ...
        return "<img src='" + img_src +"' class='hangman'>";
    }

    function generate_hangman_pane() {

        var img = select_img(state.incorrect_guesses);

        return "<div class='hangman-pane'>" + img + "</div>";
    ...
    function check_lose() {
        ...
        return state.incorrect_guesses >= 7; // if 7 incorrect guesses, lose
    ...
    function has_guessed_letter(letter='') {
        var num_guessed = state.guessed_letters.length;

        for (var j = 0 ; j < num_guessed ; j++ ) {
            if (letter == state.guessed_letters[j]) {
                return true;
            }
        }
        return false;
    ...
    function generate_user_display() {
        ...
        if (won == 0) { // no games won
            ...
            var win_perc = Math.floor((won / (won + lost) * 1000)) / 10;
            ratio = win_perc + "% won" ;
        }
    ...
    function generate_incorrect_guesses() {
        ...
        return "<span class='wrong-highlight pull-left small-text'> Incorrect Guesses: " + incorrect + "</span>"; // return display html for the incorrect guesses
    ...
    function generate_check_button() {
        ...
        return "<button type='button' class='btn btn-md btn-info' onclick='check_letter()'>CHECK</button>";
    }
